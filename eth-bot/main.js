...
function getTmpEngine() {
  if (!_tmpEngine) {
    ...
    _tmpEngine = new TradingEngine({ dryRun:true, capital:0, symbols:[{symbol:"ETH_USDT",weight:1}] }, ()=>{});
  }
  return _tmpEngine;
}
...
app.whenReady().then(() => {
  ...
  ipcMain.on("telegram:test", async (_, cfg) => {
    ...
    if (testConfig.botToken && testConfig.chatId) {
      await sendTelegramNotification("🤖 <b>ETH Bot</b>\n✅ Telegram 通知测试成功！");
    }
  });
  ...
  ipcMain.on("history:exportCSV", () => {
    ...
    const fp  = path.join(app.getPath("downloads"), `eth_trades_${Date.now()}.csv`);
    const BOM = "\uFEFF";
    const hdr = "时间,交易对,方向,原因,入场价,出场价,张数,盈亏%,盈亏U\n";
    const rows = records.map(r => {
      const dt = new Date(r.time+8*3600*1000).toISOString().replace("T"," ").slice(0,19);
      return `${dt},${r.symbol||'ETH_USDT'},${r.dir},${r.reason},${r.entry},${r.exit??""},${r.contracts},${r.pPct??""},${r.pnl??""}`;
    }).join("\n");
  ...
  ipcMain.on("bot:start", async (_, cfg) => {
    ...
    // 确保symbols配置
    if (!cfg.symbols) {
      cfg.symbols = [{ symbol: cfg.symbol || "ETH_USDT", weight: 1 }];
    }
    ...
    // 启动通知
    if (telegramConfig) {
      sendTelegramNotification(`🚀 <b>ETH Bot 已启动</b>\n${cfg.dryRun ? '🔷 模拟模式' : '🔴 真实交易'}\n📊 杠杆: ${cfg.leverage}x`);
    }
  });
  ...
  ipcMain.on("bot:stop", () => {
    ...
    if (telegramConfig) {
      sendTelegramNotification("🛑 <b>ETH Bot 已停止</b>");
    }
  });
