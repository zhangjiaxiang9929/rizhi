...
class TradingEngine {
  constructor(config, emit) {
    ...
    // ★ 增强：多币种支持
    this.symbols = config.symbols || [{ symbol: "BTC_USDT", weight: 1 }];
  ...
  async getPrice(symbol="BTC_USDT") { const r=await this.request("GET","/futures/usdt/tickers",{contract:symbol}); return parseFloat(r.body[0].last); }
  async getPosition(symbol="BTC_USDT") {
  ...
  async getContractInfo(symbol="BTC_USDT") {
  ...
  async closePosition(size,dir,symbol="BTC_USDT") {
    const r=await this.request("POST","/futures/usdt/orders",{},{
